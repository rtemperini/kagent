 ...
 */
/*
 * The summary and the two paged reads that replaced the one unbounded inventory.
 *
 * Counted from the same fixture the unpaged read uses, so the tiles and the tables
 * cannot disagree with each other — which on a real cluster they cannot either,
 * because the server counts.
 */
on(SystemService.method.getSubstrateSummary, (input, call) => {
  ...
  return {
    ...
    runningActorCount: actors.filter(
      (actor) => (actor.status ?? "").toLowerCase() === "running",
    ).length,
    busyWorkerCount: workers.filter((worker) => Boolean(worker.actorId)).length,
    actorStatusCounts: [...statusCounts.entries()]
...
on(SystemService.method.listSubstrateActors, (input, call) => {
  if (call.scenario === "empty") return { actors: [], page: {}, totalSize: 0 };

  const scope = input.namespace.trim();
  const needle = input.filter.trim().toLowerCase();
  const matching = mockSubstrateStatus.actors
    .filter(
      (actor) =>
        scope === "" ||
        !actor.actorTemplateNamespace ||
        actor.actorTemplateNamespace === scope,
    )
    // Matched against the fields a row displays, as the controller does — a search
    // that hit on something off screen would read as filtering at random.
    .filter(
      (actor) =>
        needle === "" ||
        [
  ...
  /*
   * The same sort keys the controller uses, and for the same reason: each one ends
   * in a unique tiebreaker so the page token — which is the key of the last row
   * sent — names exactly one row.
   */
  const actorKey = (actor: (typeof matching)[number]) => {
    switch (input.sortField) {
      case PbActorSortField.ACTOR_ID:
        return actor.actorId;
      case PbActorSortField.ACTOR_TEMPLATE:
        return `${actor.actorTemplateNamespace ?? ""}/${actor.actorTemplateName ?? ""}\u0000${actor.actorId}`;
      case PbActorSortField.WORKER_POD:
        return `${actor.ateomPodNamespace ?? ""}/${actor.ateomPodName ?? ""}\u0000${actor.actorId}`;
      default:
        return `${actor.status ?? ""}\u0000${actor.actorId}`;
    }
  };
  const descending = input.sortOrder === PbSortOrder.DESCENDING;
  matching.sort((left, right) => {
    const compared = actorKey(left).localeCompare(actorKey(right));
    return descending ? -compared : compared;
  });

  const { page, nextPageToken } = paged(
    matching,
    input.page?.limit ?? 0,
  ...
  return {
    ...
    page: { nextPageToken },
    // The whole matching count, not the page's length: it is what lets a caller say
    // "3 of 40" rather than implying the page is the lot.
    totalSize: matching.length,
    // The order actually applied, which is what the table reports — a fixture that
    // echoed the request would agree with a client that had ignored its own.
    appliedSortField: input.sortField,
    appliedSortOrder: descending
      ? PbSortOrder.DESCENDING
      : PbSortOrder.ASCENDING,
    computedAt: timestampFromDate(new Date()),
  };
});

on(SystemService.method.listSubstrateWorkers, (input, call) => {
  if (call.scenario === "empty") return { workers: [], page: {}, totalSize: 0 };

  const scope = input.namespace.trim();
  const needle = input.filter.trim().toLowerCase();
  const matching = mockSubstrateStatus.workers
    .filter(
      (worker) =>
        scope === "" || !worker.workerNamespace || worker.workerNamespace === scope,
    )
    .filter(
      (worker) =>
        needle === "" ||
  ...
  const pod = (worker: (typeof matching)[number]) =>
    `${worker.workerNamespace}/${worker.workerPod}`;
  const workerKey = (worker: (typeof matching)[number]) => {
    switch (input.sortField) {
      case PbWorkerSortField.POD:
        return pod(worker);
      case PbWorkerSortField.ACTOR:
        // Idle workers sort together, and after the busy ones ascending — an empty
        // string would put every available worker first.
        return `${worker.actorId || "\uffff"}\u0000${pod(worker)}`;
      default:
        return `${worker.workerPool}\u0000${pod(worker)}`;
    }
  };
  const descending = input.sortOrder === PbSortOrder.DESCENDING;
  matching.sort((left, right) => {
    const compared = workerKey(left).localeCompare(workerKey(right));
    return descending ? -compared : compared;
  });
...
/**
 * One page of a sorted list, keyed the way the controller keys it.
 *
 * The token is the sort key of the last row already sent, and it is empty on the
 * last page — so a caller never fetches an empty page to discover it has finished.
 * Shared by both paged reads so they cannot page differently from each other.
 */
function paged<T>(
  rows: readonly T[],
  limit: number,
  ...
  const size = limit > 0 ? limit : SUBSTRATE_DEFAULT_PAGE_SIZE;
  // "After the token" means *later in the chosen order*, so descending compares the
  // other way. Getting this wrong drops rows at a page boundary, silently.
  const after = (rowKey: string) =>
    descending ? rowKey < pageToken : rowKey > pageToken;
  const start = pageToken ? rows.findIndex((row) => after(key(row))) : 0;
  if (start === -1) return { page: [], nextPageToken: "" };
  const page = rows.slice(start, start + size);
  const more = start + size < rows.length;
  return {
    page,
    nextPageToken: more ? key(page[page.length - 1]) : "",
  };
}

/** What a paged substrate read answers with when the caller does not ask. */
const SUBSTRATE_DEFAULT_PAGE_SIZE = 50;
